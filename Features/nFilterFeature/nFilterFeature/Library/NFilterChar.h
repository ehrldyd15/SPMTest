...
@interface NFilterChar : UIViewController <NFilterToolbarDelegate> {
    ...
    SEL _pMethodOnInputType;        // 입력문자 타입
    ...
    BOOL isSuportBackgroundEvent;   // 백그라운드 이벤트 : 반키패드 모드에서 키패드 이외의 영역의 UI Touch 설정
    BOOL isFullMode;                // 풀모드 옵션
    BOOL isNoPadding;
    BOOL isNoSound;                 // 사운드
    BOOL isVibration;               // 진동설정
...
@property (nonatomic, strong) NFilterToolbar2Add *toolbar2Add;
...
@property (nonatomic, assign) BOOL okCancelChange;                      // 전면키패드 확인,취소 위치 교환
...
@property (nonatomic, assign) BOOL supportViewRotatation;               // 키패드 회전지원 여부 설정
...
@property (nonatomic, assign) BOOL showHanguleText;                     // 문자 키패드 한글 문자를 표시할 지 여부를 설정
@property (nonatomic, assign) BOOL showKeypadBubble;                    // 문자 툴팁 보임 여부 설정
@property (nonatomic, strong) UIImage *keypadBubbleImage;               // 문자 툴팁 이미지 변경
...
@property (nonatomic, assign) int bottomMaginForKeyPad;                 // 반키패드 하단 마진 값 설정.
@property (nonatomic, assign) int widthMaginForKeyPad;                  // 반키패드 좌우 마진 값 설정.
@property (nonatomic, assign) int bottomMaginForKeyPadLandscape;        // 반키패드 하단 마진 값 설정. (가로 모드)
@property (nonatomic, assign) int widthMaginForKeyPadLandscape;         // 반키패드 좌우 마진 값 설정. (가로 모드)
...
@property (nonatomic, assign) BOOL useVoiceOverViaSpreaker;             // VoiceOver 사용시 Speaker로 사용할지 여부 설정
@property (nonatomic, strong) NSString *closeButtonAccessibilityTxt;    // 반키패드 백그라운드 클릭시 백그라운드 버튼 대체 텍스트 설정
...
// 이하 고객지원옵션
- (void)setSupportBackgroundEvent:(BOOL)pYesOrNo;           // 백그라운드 이벤트 : 반키패드 모드에서 키패드 이외의 영역의 UI Touch 설정
...
- (void)setButtonVibration:(BOOL)pYesOrNo;                  // 진동 설정
...
- (void)setInputTypeCallbackMethod:(id)pTarget
                 methodOnInputType:(SEL)pMethodOnInputType; // 입력문자 타입 콜백 메소드 설정
...
@property (nonatomic, assign) BOOL useVoiceOverWithKorean; // 접근성 설정시 한글 읽어주는 설정 옵셥
@property (nonatomic, strong) NSString *specialButtonAccessibility;     // special 접근성 문구 설정
@property (nonatomic, strong) NSString *changeButtonAccessibility;      // change 접근성 문구 설정
